...
from Scripts.Types import Memory, Binary, CodeDistribution, LBinary
...
def generate_code(x: int, m: int) -> Binary:
    return to_LBinary(complete_zeros(to_bin(x), m))
...
def to_LBinary(x: Binary) -> LBinary:
    '''
    Input:
        x: A binary (string) code.
        
    Transform the string code into a array of integers (0, 1)
    '''
    return list(map(lambda x: int(x), list(x)))
