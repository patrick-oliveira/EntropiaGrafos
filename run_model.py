...
if __name__ == "__main__":
    parser.add_argument(
        ...
        default = None,
        dest = 'param_file',
        help = "Path for a json file containing the set of parameters. If specified, the other arguments will be ignored."
    )
    parser.add_argument(
        '--graph_type',
        type = str,
    ...
    parser.add_argument(
        ...
        help = "Control the number of new connection as new nodes are added to the network."
    )
    parser.add_argument(
        '--polarization_type',
        type = int,
    ...
    parser.add_argument(
        ...
        action = 'store_true',
        dest = 'early_stop',
        help = "Stop the replications of the experiment if the errors fall below an epsilon threshold."
    )
    parser.add_argument(
        '--epsilon',
        type = float,
    ...
    parser.add_argument(
        ...
        type = int,
        dest = 'num_processes',
        help = "Number of proccesses used to paralelyze the simulation of multiple parameter combinations."
    )
    ...
    output_path = Path("results") / param_to_hash(tuple(params.values())[:-5])
    ...
    if new_experiment:
        ...
        if converged or all_runs:
            print("This parameter combination has already been simulated up to {} or up an acceptable error threshold.".format(params["num_repetitions"]))
        else:
