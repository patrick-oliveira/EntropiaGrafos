...
class Model:
    def __init__(
        ...
        **kwargs
        ...
        The model is created by the following sequence of steps:
            ...
            1. The underlying network is built. Until now, the only network
            used is the Barabasi-Albert, but it's possible to generalize the
            model for different network topologies.

            2. The function "initialize_nodes" is called and an "Individual"
            object is instantiated and attributed to each node in the network.

            3. The individuals are grouped in 3 different groups, one for
            individuals who polarize upwards, another for individuals who
            polarize downwards, and another for neutral individuals. The
            proportion of individuals for each group is defined by the
            parameters alpha and omega.

            4. The function "update_node_info" is called to compute
            polarization probabilities. The same function is used to update
            the model after each iteration, therefore there is a loop calling
            the "update_memory" function for each node in the network.

            5. The function "compute_edge_weight" is called to attribute
            weights to each edge using the Jensen-Shannon Divergence.

            6. The function "compute_sigma_attribute" is called to measure the
            popularity of each node based on the connection weight with each
            neighbour.

            7. The function "compute_graph_entropy" is called to calculate the
            mean entropy of the network.

            8. The function "compute_graph_polarity" is called to calculate
            the mean polarity of the network.

        The steps 4 to 8 are repeated at each iteration to update the model's
        values.

        Args:
            N (int): Number of nodes in the network.
            pa (int): 'Preferential Attachment' parameters (exclusive to the
            Barabasi-Albert network; the choice of network topology can be
            generalized later)
            mu (int): Memory size.
    ...
    def create_graph(self):
        """
        Edite esta função para permitir a criação de redes com topologias
        diferentes. A função deve receber um nome identificando o tipo de rede,
        e/ou uma função de criação da rede.
        """
    ...
    def initialize_model_info(self):
        """
        Initialize the model by creating the individual's information and
    ...
    def group_individuals(self):
        """
        Individuals are randomly grouped accordingly to their polarization
        tendencies.
        """
        if self.polarization_grouping_type == 0:
            # totally random
            indices = sample(list(range(self.N)), k = self.N)
        elif self.polarization_grouping_type == 1:
            # most connected individuals are polarized
            indices = list(
                np.argsort([self.G.degree[x] for x in range(self.N)])
            )
            indices.reverse()
        elif self.polarization_grouping_type == 2:
    ...
    def compute_mean_edge_weight(self) -> None:
        ...
        None.
        '''
        distances = [self.G[u][v]['Distance'] for u, v in self.G.edges]
        self._J = sum(distances) / self.E
